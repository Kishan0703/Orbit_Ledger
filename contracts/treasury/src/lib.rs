...
use soroban_sdk::{
    ...
    symbol_short, Address, Env, IntoVal, Symbol, Vec, token,
};
...
impl TreasuryContract {
    ...
    /// Link the SPAY Token contract address. Call once after deployment.
    pub fn set_spay_token(env: Env, admin: Address, spay_token_addr: Address) {
        admin.require_auth();
        env.storage().instance().set(&DataKey::SpayToken, &spay_token_addr);
        env.events().publish((symbol_short!("spay_set"),), spay_token_addr);
    }
    ...
    pub fn execute_proposal(
        ...
        // 🟢 LEVEL 4: Inter-contract call — mint 1 SPAY per employee paid
        let spay_addr: Address = env
            .storage()
            .instance()
            .get(&DataKey::SpayToken)
            .ok_or(TreasuryError::SpayTokenNotSet)?;

        for i in 0..proposal.employees.len() {
            let employee = proposal.employees.get(i).unwrap();
            let mint_args: Vec<soroban_sdk::Val> = Vec::from_array(
                &env,
                [employee.into_val(&env), 1i128.into_val(&env)],
            );
            env.invoke_contract::<()>(
                &spay_addr,
                &Symbol::new(&env, "mint"),
